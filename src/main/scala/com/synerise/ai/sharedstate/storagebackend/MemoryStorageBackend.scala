...
case class MemoryStorageBackend(stack: Map[SharedStateKey, SharedState]) extends StorageBackend {
  def update(sharedState: SharedState, processVersion: Boolean) = {
    val currentSharedState = stack.get(sharedState.key) match {
      case Some(currentSharedState) if currentSharedState.version == sharedState.version && processVersion || !processVersion  =>
        Some(if (processVersion) sharedState.updateVersion else sharedState)
